...
public class AudioAnalyzer : MonoBehaviour
{
    ...
    public static float GetScaledOutput(int listenBand, float bandMax, float targetMin, float targetMax)
    {
        ...
        float rawOutput = output[listenBand];
        float scaledOutput = output[listenBand].Map(0, bandMax, targetMin, targetMax);

        return scaledOutput;
    }

    public static float GetOutput(int listenBand)
    {
        if (disabled)
        {
            return 0;
    ...
    public IEnumerator Record(int seconds, string filename)
    {
        if (seconds > 0 && !filename.Equals(""))
        {
            source.Stop();
            source.clip = Microphone.Start(selectedDevice, true, seconds, maxFreq);
            while (Microphone.GetPosition(selectedDevice) <= 0)
                yield return Microphone.GetPosition(selectedDevice);
            source.Play();
            yield return new WaitForSeconds(seconds);
            SaveAudio.Save(filename, source.clip);
        }
    }
    #endregion
        ...
        for (int i = 0; i < freqData.Length; i++)
        {
            ...
            if (i > crossovers[k])
            {
                if (easeAmplitude)
                {
                    ...
                    output[k] = Mathf.Lerp(output[k], bandAmp * masterGain, (bandAmp > output[k] ? bandAmp * climbRate : output[k] *  fallRate)); // if analyzed amplitude is larger than previous amplitude, ease to new amplitude at climbRate, otherwise ease at fallRate
                }
                else
